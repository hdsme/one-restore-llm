...
class Dataset(udata.Dataset):
    ...
    def __getitem__(self, index):
        ...
        caption = h5f[key].attrs["caption"]
        h5f.close()
        return torch.Tensor(data), caption
        ...
        for i in range(num_raw):
            for j in range(num_col):
                if stride * i + win <= raw and stride * j + win <= col:
                    img_patches[:, :, :, :, count] = img[:, :, stride *
                                                         i: stride*i + win, stride*j: stride*j + win]
                elif stride * i + win > raw and stride * j + win <= col:
                    img_patches[:, :, :, :, count] = img[:, :,
                                                         raw-win: raw, stride * j: stride * j + win]
                elif stride * i + win <= raw and stride*j + win > col:
                    img_patches[:, :, :, :, count] = img[:, :,
                                                         stride*i: stride*i + win, col-win: col]
                else:
                    img_patches[:, :, :, :, count] = img[:,
                                                         :, raw-win: raw, col-win: col]
                img_patches[:, :, :, :, count] = data_augmentation(
                    img_patches[:, :, :, :, count], np.random.randint(0, 7))
                count += 1
    return img_patches
...
def Train_data(args):
    ...
    with h5py.File(args.data_name, 'w') as h5f:
        ...
        for i in range(len(file_list)):
            ...
            img_list.append(
                read_img(f'{args.train_path}/{args.gt_name}/{file_list[i]}'))
            for j in args.degradation_name:
                img_list.append(
                    read_img(f'{args.train_path}/{j}/{file_list[i]}'))

            img = np.stack(img_list, 0)
            img = img_to_patches(img.transpose(0, 3, 1, 2),
                                 args.patch_size, args.stride)

            caption_text = get_dynamic_label(mode='train', degradation=j, filename=file_list[i])
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Building the training patch database")
    ...
    parser.add_argument("--train-path", type=str,
                        default='./data/CDD-11_train', help="Train path")
    parser.add_argument("--data-name", type=str,
                        default='dataset.h5', help="Data name")
